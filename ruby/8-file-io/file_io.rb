...
puts "File.exist?(file)       : #{File.exist?(file)}"
puts "File.file?(file)        : #{File.file?(file)}"
puts "File.directory?(file)   : #{File.directory?(file)}"
puts "File.readable?(file)    : #{File.readable?(file)}"
puts "File.writable?(file)    : #{File.writable?(file)}"
puts "File.executable?(file)  : #{File.executable?(file)}"
puts "File.size(file)         : #{File.size(file)}"
puts "File.dirname(file)      : #{File.dirname(file)}"
puts "File.expand_path(file)  : #{File.expand_path(file)}"
full_path = File.expand_path(file)
puts "File.basename(full_path): #{File.basename(full_path)}"
puts "File.extname(file)      : #{File.extname(file)}"

puts "File.atime(file)        : #{File.atime(file)}"  # last accessed time, read or write
puts "File.mtime(file)        : #{File.mtime(file)}"  # last modified time, write
puts "File.ctime(file)        : #{File.ctime(file)}"  # last status change time, NOT created time
...
puts "Output of raven.txt"
File.open('raven.txt', 'r') do |file|
